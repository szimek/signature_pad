...
class SignaturePad {
    ...
    fromSkiaPath(skiaPath) {
        ...
        const maxY = this.canvas.height / ratio;
        const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
        const skiaPointGroups = skiaPath
            .split(';')
        ...
        for (const skiaPointGroup of skiaPointGroups) {
            const startCoords = skiaPointGroup.path
                .split(' C ')[0]
                .substring(1)
                .trim()
                .split(',')
                .map(i => parseFloat(i));
            const startPoint = new Point(startCoords[0], startCoords[1]);
            const endCoords = skiaPointGroup.path
                .split(' C ')[1]
                .split(' ')
                .pop()
                .split(',')
                .map(i => parseFloat(i));
            const endPoint = new Point(endCoords[0], endCoords[1]);
